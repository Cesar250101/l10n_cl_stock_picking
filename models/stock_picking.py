...
class StockPicking(models.Model):
    ...
    def _giros_emisor(self):
        giros_emisor = []
        for acteco in self.picking_type_id.warehouse_id.acteco_ids or self.company_id.company_activities_ids:
            giros_emisor.append(acteco.code)
        return giros_emisor
    ...
    def _id_doc(self, resumen):
        ...
        if resumen['tax_include'] and resumen['MntExe'] == 0 :
            IdDoc['MntBruto'] = 1
    ...
    def _picking_lines(self):
        ...
        picking_lines = []
        MntExe = 0
        interna = self.move_reason in ['5']
        product = not interna
        for line in self.move_lines:
            product = not interna and line.product_id.default_code != 'NO_PRODUCT'
            lines = {}
            ...
            qty = 0
            if product:
                if line.product_id.default_code or line.product_id.barcode:
                    lines["CdgItem"] = []
                ...
                MontoItem = int(round(subtotal,0))
            lines['QtyItem'] = qty
    ...
    def _dte(self, n_atencion=None):
        ...
        dte['Encabezado'] = self._encabezado(picking_lines)
        lin_ref = 1
        ref_lines = []
